...
from ide.components.menu_bar import run_code
...
def configure_keyboard_shortcuts(root, notebook, add_tab_callback):
    ...
    """
    root.bind(SHORTCUTS["new_file"], 
              lambda event: new_file(notebook, add_tab_callback))  # Ctrl+N
    root.bind(SHORTCUTS["open_file"], 
              lambda event: open_file(notebook, add_tab_callback))  # Ctrl+O
    ...
    root.bind(SHORTCUTS["run_code"],
              lambda event: run_code(notebook))  # Ctrl+E
